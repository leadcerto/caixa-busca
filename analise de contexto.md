# 📋 VISÃO GERAL DO SISTEMA — Gerenciador de Estoque de Imóveis

## 🎯 Objetivo do Sistema

Plataforma web para **captação, organização e distribuição de leads** interessados em imóveis à venda, acessível pelo endereço **venda.imoveisdacaixa.com.br**.

O sistema conecta **compradores em potencial** (leads) às **imobiliárias parceiras** responsáveis pelo atendimento, sem que as imobiliárias precisem operar ativamente dentro da plataforma.
...
- Visualiza os leads recebidos: data, nome, e-mail, imóvel de interesse
...
- A frequência de importação não é fixa — pode ocorrer mais de uma vez por semana ou nenhuma, conforme disponibilidade do arquivo
- O sistema processa o arquivo e atualiza a base de dados com os imóveis disponíveis
- Cada imóvel contém informações como: endereço, cidade, estado, valor, modalidade de venda, tipo de imóvel, link do edital, entre outros
- Os imóveis são **vinculados a uma imobiliária parceira** responsável pelo atendimento do estado correspondente
...
- Número do imóvel (chave principal — único)
...
4. **Desmembramento da descrição** — extração via PHP dos campos individuais contidos no texto original (tipo, área, quartos, características)
5. **Scraping** — coleta de informações complementares (FGTS, dados adicionais)
...
O campo de descrição original contém múltiplas informações em texto livre. O sistema extrai automaticamente via PHP os seguintes campos:
...
A localização é estruturada em níveis relacionais para evitar duplicação e permitir enriquecimento de conteúdo:
...
Cada município e bairro pode ter seu conteúdo enriquecido por Inteligência Artificial, armazenado em um campo JSON. O objetivo é fornecer informações relevantes para compradores que não conhecem a região.
...
- O link compartilhado gera uma **pré-visualização** no WhatsApp com a imagem de destaque configurada
...
- Esse formulário gera um **lead vinculado ao imóvel e à imobiliária responsável pelo estado**
- A imobiliária é **notificada automaticamente** por e-mail e WhatsApp
- Se o mesmo lead se interessar por múltiplos imóveis, a imobiliária recebe uma notificação separada para cada imóvel
- O histórico de imóveis de interesse do lead fica **gravado em JSON** dentro do registro do lead:
...
🖼️ IMAGENS
📸 Foto da Fachada
Única foto real do imóvel (tirada da rua, da fachada)
Hospedada em servidor externo
O sistema armazena apenas a URL que aponta para essa imagem
🔗 Imagem de Destaque (Open Graph)
Imagem genérica / institucional do site
Aparece quando um link é compartilhado no WhatsApp ou redes sociais
Serve como convite visual para o usuário clicar no link
Hospedada no servidor próprio
⚙️ REGRAS DE NEGÓCIO PRINCIPAIS
Cada imóvel está vinculado a uma imobiliária por estado
O lead é enviado somente para a imobiliária responsável pelo estado do imóvel
A imobiliária não acessa o sistema operacionalmente — apenas visualiza
O lead precisa confirmar o e-mail para ser considerado válido
Áreas restritas do site só são acessíveis para leads confirmados e logados
Os campos originais do CSV são preservados integralmente e nunca alterados
O valor do desconto em reais é calculado pelo sistema: desconto_valor = valor_avaliacao - valor_venda
O histórico de atualizações de cada imóvel é mantido para análise de evolução de preço e tempo de mercado
O campo FGTS tem valor inicial "não informado" até que o scraping seja realizado
O campo MCMV está reservado para uso futuro
A data de referência corresponde à data de geração do CSV, não à data de importação